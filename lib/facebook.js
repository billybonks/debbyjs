...
const { arrayHasValues, unique, compact } = require('./util');

function fbQuickReplies(quickreplies) {
  if (!quickreplies || !Array.isArray(quickreplies)) return;
  quickreplies = unique(compact(quickreplies));

  return arrayHasValues(quickreplies) && quickreplies.map((quickreply) => {
